# Class charging
# Contains all data of a vehicle charging process
...
from .exceptions import RawDataError, HistogramError
from .histogram import Histogram, Histogram2D, Bins


class Charging(BEVUsage):
    # Hyperparameter
    IMPLEMENTED_HISTOGRAMS = [
        ...
        HistogramType.CHG_EOC,
        HistogramType.CHG_DURATION,
        ...
        HistogramType.CHG_HIST2D_POWER__PACK_TEMP_MAX,
        HistogramType.CHG_HIST2D_POWER__PACK_TEMP_DELTA,
        HistogramType.CHG_HIST2D_POWER__SOC,
    ...
    def __init__(self, charging_id, start, end, vehicle, processed, prior_usage_end=None):
        super().__init__(database_id=charging_id, start=start, end=end, vehicle=vehicle, processed=processed,
    ...
    def analyze(self, desired_histograms: List[HistogramType], plot=False):
        ...
        :return:
        """
        logging.info("Analyzing charging process '" + str(self.id) + "'...")
        ...
        if len(remaining_histogram_types) == len(desired_histograms):
            ...
            logging.info("--> All requested histograms for this charging process has been already calculated!")
            return
        else:
        ...
        if not self.is_complete():
            raise RawDataError(
                "Charging process is not complete and therefore can not be processed right now. Try again later.")

        # Verify charging process
        if not self._verified():
            raise RawDataError("Charging process contains invalid data and therefore can not be processed.")
    ...
    def save_histograms_to_database(self, force_overwrite=False):
        ...
        :param force_overwrite: Bool, overwrite histograms in database?
        :return:
        """
        ...
        sql = '''SELECT value_id_x, value_id_y, road_type FROM track.charging_ev_histograms WHERE charging_id=%s;'''
        ...
        for hist in self.histogram_list:
            if type(hist) == Histogram:
                if (hist.data_type.id, 0, hist.road_type.name.lower()) in existing_histograms and not force_overwrite:
                    ...
                    sql_query = '''UPDATE track.charging_ev_histograms SET 
                    counts = %s, bins_id_x = %s, bins_id_y = 0, time_computed = now(), road_type = %s 
                    WHERE charging_id=%s AND value_id_x=%s AND value_id_y=0'''
                    sql_parameter = [hist.counts.tolist(), bins_id, hist.road_type.name.lower(), self.id,
                                     hist.data_type.id]
                else:
                    if hist.bins in existing_bins_dict.values():
                        bins_id = self._get_key_for_value(existing_bins_dict, hist.bins)
                    ...
                    sql_query = '''INSERT INTO track.charging_ev_histograms 
                            (charging_id, value_id_x, value_id_y, counts, bins_id_x, bins_id_y, time_computed, road_type) 
                            VALUES (%s, %s, 0, %s, %s, 0, now(), %s)'''
                    sql_parameter = [self.id, hist.data_type.id, hist.counts.tolist(), bins_id,
                                     hist.road_type.name.lower()]
                cur.execute(sql_query, sql_parameter)
                conn.commit()
                ...
                if (hist.x_data_type.id, hist.y_data_type.id, hist.road_type.name.lower()) in existing_histograms \
                    ...
                    sql_parameter = [hist.counts.tolist(), bins_id_x, bins_id_y, hist.road_type.name.lower(), self.id,
                                     hist.x_data_type.id, hist.y_data_type.id]
                else:
                    # Check if x bins already exist
                    if hist.x_bins in existing_bins_dict.values():
                        bins_id_x = self._get_key_for_value(existing_bins_dict, hist.x_bins)
                    ...
                    sql_parameter = [self.id, hist.x_data_type.id, hist.y_data_type.id, hist.counts.tolist(), bins_id_x,
                                     bins_id_y, hist.road_type.name.lower()]
                cur.execute(sql_query, sql_parameter)
                conn.commit()
        cur.close()
        conn.close()
    ...
    def calculate_histogram(self, histogram_type: HistogramType, plot=False, keep_cache=False):
        ...
        # Temperatures
            ...
            if "max_temp" in self.cache.keys() and "min_temp" in self.cache.keys():
                ...
                timeseries = Charging._add_data_gaps_to_timeseries(self.can_data["hv_battery_current"], timeseries)
                ...
                for i in range(len(pack_temp_data)):
                    pack_temp_data_in_timeseries.append(
                        Charging._match_data_to_timeseries(pack_temp_data[i], timeseries))
                ...
                max_temp = Charging._match_data_to_timeseries(raw_max_temp, timeseries)
                min_temp = Charging._match_data_to_timeseries(raw_min_temp, timeseries)
        ...
        # Elevation of charge
            ...
            eoc = self.can_data["hv_soc"][-1] - self.can_data["hv_soc"][0]
            eoc_hist = self._create_custom_histogram(data_type=self.value_id_dict[1298],
                                                     data=[eoc],
                                                     bins=self.BINS_FOR_HISTOGRAMTYPE[histogram_type][0])
            if plot:
                eoc_hist.plot(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(eoc_hist)
            logging.info("--> Calculated histogram of EOC successfully.")
        ...
        # Charge duration
            ...
            duration_hist = self._create_custom_histogram(data_type=self.value_id_dict[1296],
                                                          data=[Datapoint(variable_name="charge_duration",
            ...
            logging.info("--> Calculated histogram of charge duration successfully.")
        ...
        # battery voltage
            ...
            hv_voltage_hist = self._create_histogram(data_type_name="hv_battery_voltage",
                                                     bins=self.BINS_FOR_HISTOGRAMTYPE[histogram_type][0])
            if plot:
            ...
            if "max_cell_voltage" in self.cache.keys() and "min_cell_voltage" in self.cache.keys():
                ...
                for i in range(len(cell_voltage_data)):
                    cell_voltage_data_in_timeseries.append(
                        Charging._match_data_to_timeseries(cell_voltage_data[i], timeseries))
        ...
        # charge power
            ...
            if "hv_battery_current" not in self.can_data.keys() or "hv_battery_voltage" not in self.can_data.keys():
                raise RawDataError("Missing data for calculation!")

            hv_voltage = self.can_data["hv_battery_voltage"]
            hv_current = self.can_data["hv_battery_current"]

            # Use timeseries to match correct voltage and current values
            data = [hv_voltage, hv_current]
            ...
            timeseries = Charging._add_data_gaps_to_timeseries(hv_current, timeseries)

            # Match data to timeseries
            data_in_timeseries = []
            for i in range(len(data)):
                data_in_timeseries.append(
                    Charging._match_data_to_timeseries(data[i], timeseries))
            ...
            for i in range(len(data_in_timeseries[0])):
                ...
                if current_i.value >= 0:
                    charge_power.append(Datapoint(variable_name="charge_power",
                                                  variable_value=float(voltage_i.value) * float(current_i.value) / 1000,
                                                  time_of_measurement=timeseries[i],
                                                  road_type=current_i.road_type))
                elif current_i.value < 0 and i / len(data_in_timeseries[1]) >= 0.98:
                    logging.warning("Negative current value (" + str(current_i.value) + "A) during charging, but in the"
                                                                                        " last 2% of charging process ("
                                    + str(np.around((i / len(data_in_timeseries[1])) * 100, 2)) + "% completed). If "
                                                                                                  "charging is finished and some idle data is tracked, this warning can be ignored.")
                else:
                    raise RawDataError("Negative current during charging should not occur!")

            self.cache["power"] = charge_power

            power_hist = self._create_custom_histogram(data_type=self.value_id_dict[1297],
                                                       data=charge_power,
                                                       bins=self.BINS_FOR_HISTOGRAMTYPE[histogram_type][0])
            if plot:
                power_hist.plot(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(power_hist)
            logging.info("--> Calculated histogram of charge power successfully.")
        ...
        # coolant temperature at inverter inlet
            ...
            ambient_air_hist = self._create_histogram(data_type_name="coolant_temp_inverter_inlet",
                                                      bins=self.BINS_FOR_HISTOGRAMTYPE[histogram_type][0])
            if plot:
                ambient_air_hist.plot(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(ambient_air_hist)
        ...
        # coolant temperature at pack inlet
            ...
            hv_battery_temp_inlet_hist = self._create_custom_histogram(data_type=self.value_id_dict[1272],
                                                                       ...
                                                                       bins=self.BINS_FOR_HISTOGRAMTYPE[histogram_type][
                                                                           0])
            if plot:
        ...
        # rear inverter temperatur
            ...
            temp_rear_inverter_inlet_hist = self._create_custom_histogram(data_type=self.value_id_dict[963],
                                                                          ...
                                                                          bins=
                                                                          self.BINS_FOR_HISTOGRAMTYPE[histogram_type][
                                                                              0])
            if plot:
        ...
        # ptc1 power
            ...
            data_in_timeseries = []
            for i in range(len(data)):
                data_in_timeseries.append(Charging._match_data_to_timeseries(data[i], timeseries))
        ...
        # 2D pack temp max X soc
            ...
            if "hv_soc" not in self.can_data.keys() or len(self.can_data["hv_soc"]) < 2:
                raise RawDataError("Missing data for calculation!")
        ...
        # 2D charge power X max pack temp
            ...
            if "max_temp" in self.cache.keys():
                ...
                self.histogram_list.pop()
                max_temp = self.cache["max_temp"]
            # Check if charge power data is cached. If not, calculate.
            if "power" in self.cache.keys():
                charge_power = self.cache["power"]
            else:
                self.calculate_histogram(HistogramType.CHG_POWER, keep_cache=True)
                self.histogram_list.pop()
                charge_power = self.cache["power"]

            # Create histogram
            power_packtempmax_2dhist = self._create_custom_2d_histogram(x_data_type=self.value_id_dict[1297],
                                                                        y_data_type=self.value_id_dict[1209],
                                                                        x_data=charge_power,
                                                                        y_data=max_temp,
                                                                        x_bins=self.BINS_FOR_HISTOGRAMTYPE[
                                                                        ...
                                                                        y_bins=self.BINS_FOR_HISTOGRAMTYPE[
                                                                            histogram_type][1])
            if plot:
                power_packtempmax_2dhist.plot_2d(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
                power_packtempmax_2dhist.plot_3d(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(power_packtempmax_2dhist)
            logging.info("--> Calculated 2D histogram of charge power and maximal pack temperature successfully.")

        # 2D charge power X pack temp delta
            ...
            crate_packtempdelta_2dhist = self._create_custom_2d_histogram(x_data_type=self.value_id_dict[1297],
                                                                          y_data_type=self.value_id_dict[1289],
                                                                          x_data=charge_power,
                                                                          y_data=list(
                                                                              np.subtract(max_temp, min_temp)),
            ...
            if plot:
                ...
                crate_packtempdelta_2dhist.plot_3d(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(crate_packtempdelta_2dhist)
            logging.info("--> Calculated 2D histogram of charge power and pack temperature difference successfully.")

        # 2D pack charge power X soc
            ...
            # Create histogram
            power_soc_2dhist = self._create_custom_2d_histogram(x_data_type=self.value_id_dict[1297],
                                                                y_data_type=self.value_id_dict[900],
                                                                x_data=charge_power,
                                                                y_data=self.can_data["hv_soc"],
                                                                x_bins=self.BINS_FOR_HISTOGRAMTYPE[
                                                                ...
                                                                y_bins=self.BINS_FOR_HISTOGRAMTYPE[
                                                                    histogram_type][1])
            if plot:
                power_soc_2dhist.plot_2d(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
                power_soc_2dhist.plot_3d(title_prefix + self.TITLE_FOR_HISTOGRAMTYPE[histogram_type])
            self.histogram_list.append(power_soc_2dhist)
            logging.info("--> Calculated 2D histogram of charge power and SOC successfully.")
    ...
    def _check_if_already_processed(self, histogram_types: List[HistogramType]):
        ...
        cur.execute('''SELECT DISTINCT charging_id FROM track.charging_ev_histograms;''')
        processed_track_ids = cur.fetchall()

        # Get all already processed tracks
        sql = '''SELECT value_id_x, value_id_y, road_type, bins_id_x, bins_id_y
        ...
        for hist_type in histogram_types:
            ...
            for calc_hist in calculated_histograms:
                ...
                if calc_hist[0] == hist_val_id_x and calc_hist[1] == hist_val_id_y:
                    ...
                    calc_bins_x = Bins(maximum=db_calc_bins_x[0], minimum=db_calc_bins_x[1],
                                       step_size=db_calc_bins_x[2])
    ...
    def _verified(self):
        """
        Verifies the charging process and removes sensor errors where possible
        """
        # Make sure, all data is available
        if len(self.value_id_dict.keys()) == 0:
            self.get_value_id_data()
        ...
        new_hv_battery_current = []
        invalid_counter = 0
        for dp in self.can_data["hv_battery_current"]:
            # Check if there are discharge currents bigger than 0.05C in the data
            if abs(dp.value) >= self.vehicle.hv_cap * 0.05 and dp.value < 0:
                logging.warning(
                    "--> Charging process includes negative current bigger than 0.05C ({}A)!".format(dp.value))
                return False

            # Remove negative current data if its absolute value is below 0.05C
            if dp.value < 0 and abs(dp.value) < self.vehicle.hv_cap * 0.05:
                invalid_counter += 1
                continue
            new_hv_battery_current.append(dp)
        if invalid_counter > 0:
            logging.warning(
                "--> Removed {} invalid datapoints (I < 0 and abs(I) < 0.05C) from current data ({}% of total"
                " data).".format(invalid_counter,
                                 np.around((invalid_counter / len(self.can_data["hv_battery_current"]) * 100), 2)))

        self.can_data["hv_battery_current"] = new_hv_battery_current
        return True
